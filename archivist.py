...
def doMagic(osversion, radioversion, softwareversion, localdir, radios="y", received="y", deleted="y", hashed="y"):
...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        parser = argparse.ArgumentParser(description="Download bar files, create autoloaders.", usage="%(prog)s OSVERSION RADIOVERSION SWVERSION [options]")
        ...
        parser.add_argument("--folder", dest="dir", default=os.getcwd(), help="Directory to use; default = local", action="store_const", const=os.getcwd())
        ...
        doMagic(args.os, args.radio, args.swrelease, args.dir, args.radloaders, args.compress, args.delete, args.verify)
    else:
        localdir = os.getcwd()
        ...
        print(" ")
        doMagic(osversion=osversion, radioversion=radioversion, softwareversion=softwareversion, localdir=localdir, radios=radios, received=received, deleted=deleted, hashed=hashed)
